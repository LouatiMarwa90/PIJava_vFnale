...
public class ModifierAvisEquipement {
    ...
    private AvisEquipement currentAvis;
    ...
    // Méthode pour définir l'avis à modifier
    public void setAvis(AvisEquipement avis) {
        this.currentAvis = avis;
        loadAvisDetails();
    }

    // Méthode pour charger les détails de l'avis dans les champs
    private void loadAvisDetails() {
        if (currentAvis != null) {
            commentaire.setText(currentAvis.getCommentaire());
        }
    }
    ...
    void update(ActionEvent event) {
        try {
            ...
            currentAvis.setCommentaire(commentaire.getText().trim());

            avisEquipementService.modifier(currentAvis);
            showAlert(Alert.AlertType.ERROR, "Succès", "Avis modifié avec succès.");
        } catch (Exception e) {
            showAlert(Alert.AlertType.ERROR, "Erreur", "Erreur lors de la modification de l'avis : " + e.getMessage());
